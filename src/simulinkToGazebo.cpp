...
void chatterCallbackRef(const std_msgs::Float64MultiArray& msg) 
{
    // std::cout<<"raw="<< (double)msg.data[0]<<" conveted="<<msg.data[1]<<"\n";
  
  

  _slewAngleRef.data=msg.data[0];
  _alphaAngleRef.data=msg.data[1];
  _betaAngleRef.data=msg.data[2];
  _ext1Ref.data=msg.data[3];
  _ext2Ref.data=msg.data[4];
  _ext3Ref.data=msg.data[5];
  _ext4Ref.data=msg.data[6];
  _ext5Ref.data=msg.data[7];
  _ext6Ref.data=msg.data[8];
//  std::cout<<" Ext6="<<ext6<<" Ext5="<<ext5<<" Ext4="<<ext4<<" Ext3="<<ext3<<" Ext2="<<ext2<<" Ext1="<<ext1<<"\n";
//   std::cout<<" conveted="<<slewAngleRef<<" raw="<< (double)msg.data[1]<<" conveted="<<alphaAngleRef<<" raw="<< (double)msg.data[2]<<" conveted="<<betaAngleRef<<"\n";
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber subCC = n.subscribe("/simulink/jointRef", 1000, chatterCallbackRef);
  ros::Subscriber subCMode = n.subscribe("/simulink/control_mode", 1000, chatterCallbackMode);
  ros::Subscriber sub = n.subscribe("/Simulin/leverCmd", 1000, chatterCallback);
  ...
  ros::Rate loop_rate(100);
    ...
    if(controlMode==1){
          ...
          slewRef_pub.publish(_slewAngleRef);
          alphaRef_pub.publish(_alphaAngleRef);
          betaRef_pub.publish(_betaAngleRef);
          ext1Ref1_pub.publish(_ext1Ref);
          ext1Ref2_pub.publish(_ext2Ref);
          ext1Ref3_pub.publish(_ext3Ref);
          ext1Ref4_pub.publish(_ext4Ref);
          ext1Ref5_pub.publish(_ext5Ref);
          ext1Ref6_pub.publish(_ext6Ref);    
